...
images = [Path(f'./cards/{val}_of_{suit}.png') for suit in suits for val in values]
w, h = (110, 160)
...
with Image.open('./cards/back.png') as back:
    sprite.paste(back.resize((w, h)), (0, 4*h))
with Image.open('./cards/black_joker.png') as black_joker:
    sprite.paste(black_joker.resize((w, h)), (1*w, 4*h))
with Image.open('./cards/red_joker.png') as red_joker:
    sprite.paste(red_joker.resize((w, h)), (2*w, 4*h))
aces = [Path(f'./cards/mirror_of_{suit}.png') for suit in suits]
...
# sprite.show()
sprite.save(Path("src/assets/cards.png"))
